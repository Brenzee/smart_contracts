...
contract ERC721Exchange {
    ...
    bytes32 public constant SALE_TYPEHASH =
        keccak256("Sale(address token,uint256 tokenId,uint256 price,uint256 expiresAt)");
    ...
    function buy(Sale calldata sale, bytes32 r, bytes32 s, uint8 v) public payable {
        ...
        address nftOwner = IERC721(sale.token).ownerOf(sale.tokenId);

        require(signer == nftOwner, "ERC721Exchange: signer is not owner");
    ...
    function getSigner(Sale calldata sale, bytes32 r, bytes32 s, uint8 v) public view returns (address signer) {
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(SALE_TYPEHASH, sale.token, sale.tokenId, sale.price, sale.expiresAt))
            )
        );
